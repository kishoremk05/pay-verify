...
import { Banknote, Clock, AlertTriangle, TrendingUp, Users, RefreshCw, Layers, ShieldAlert, CheckCircle, RotateCcw, ClipboardList, FileText as FileTextIcon, CreditCard, BarChart3, X, ChevronRight, Sparkles } from "lucide-react";
...
function DashboardPage() {
  ...
  const { data: onboardingData } = useQuery({
    ...
    enabled: !!organization?.id,
    queryFn: async () => {
      const [{ count: svcCount }, { count: custCount }, { count: invCount }, { count: payCount }] = await Promise.all([
  ...
  const onboardingSteps = [
    { title: "Create Services", description: "Set up your rate card with coaching, hostel, library fees.", done: (onboardingData?.services ?? 0) > 0, href: "/services" },
    { title: "Add Customers", description: "Register the clients you will bill.", done: (onboardingData?.customers ?? 0) > 0, href: "/customers" },
    { title: "Create Invoice", description: "Generate your first payment request.", done: (onboardingData?.invoices ?? 0) > 0, href: "/invoices" },
    { title: "Import Payments", description: "Upload a payment file or record a manual entry.", done: (onboardingData?.payments ?? 0) > 0, href: "/payments" },
    { title: "View Reports", description: "Check analytics and audit reports.", done: false, href: "/reports" },
  ];

  const stepsCompleted = onboardingSteps.filter((s) => s.done).length;
  const showWizard = !wizardDismissed && stepsCompleted < 5;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {showWizard && (
        <Card className="relative border-primary/30 bg-gradient-to-br from-primary/5 via-card to-primary/5 shadow-[var(--shadow-card)] rounded-2xl overflow-hidden">
          <button onClick={() => setWizardDismissed(true)} className="absolute top-4 right-4 h-7 w-7 rounded-full bg-muted/60 hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
          ...
          <CardContent className="p-6 sm:p-8">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="text-lg font-extrabold tracking-tight text-foreground font-sans">Getting Started</h2>
                <p className="text-xs text-muted-foreground">{stepsCompleted} of {onboardingSteps.length} steps completed</p>
              </div>
            </div>
            <div className="w-full bg-muted/40 rounded-full h-2 mb-6 overflow-hidden">
              <div className="bg-primary h-full rounded-full transition-all duration-500" style={{ width: `${(stepsCompleted / onboardingSteps.length) * 100}%` }} />
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {onboardingSteps.map((step, idx) => (
                <Link key={idx} to={step.href}>
                  <div className={`group p-4 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-md ${
                    step.done
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs font-bold ${step.done ? "text-emerald-600 dark:text-emerald-400 line-through" : "text-foreground"}`}>{step.title}</span>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    {isLoading ? (
                      ...
                      <p className="text-2xl font-black text-foreground tracking-tight">{formatCurrency(k.value, currency)}</p>
                    )}
                  </div>
                ...
                return (
                  <div key={p.id} className="flex items-center justify-between p-3.5 rounded-xl border border-border/40 hover:bg-muted/30 transition-colors duration-150">
                    ...
                    <div className="text-right flex flex-col items-end gap-1.5">
                      <p className="font-extrabold text-sm text-foreground">{formatCurrency(Number(p.amount_paid), currency)}</p>
